...
class DioClient with DioMixin implements Dio {
  ...
  DioClient() : baseUrl = APIRoutes.baseUrl {
    ...
    options
      ..baseUrl = baseUrl
      ..headers = {'Content-Type': 'application/json; charset=UTF-8', 'lang': "en", "api_password": "property_bashar"};
    interceptors.addAll([
      InterceptorsWrapper(
        onRequest: (options, handler) {
          var storageService = getIt<IStorageService>();
          if (storageService.registeredUser) {
            options.headers.addAll({'AUTHORIZATION': addBearer(storageService.token ?? '')});
          }
      ...
      switch (exception.type) {
        ...
        case DioExceptionType.badResponse:
          ...
          if (response == null || response is! Response<T>) {
            ...
            // for Dio.
            throw AppNetworkResponseException(exception: exception, message: hendleMessage(exception.response?.data));
          }
  ...
  String hendleMessage(dynamic exceptionResponse) => (exceptionResponse is Map && exceptionResponse['message'] != null)
